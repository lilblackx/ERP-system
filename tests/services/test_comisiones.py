...
from app.db.models import BancoMovimiento, CajaMovimiento, ComisionFactura, FacturaDetalle
...
def _crear_comisiones_pendientes(session, vendedor, admin, cantidades_precios: list[tuple[Decimal, Decimal, Decimal]]):
    ...
    asi que hace falta una linea real por comision). Devuelve la lista de ComisionFactura
    creadas."""
    items = []
...
def test_pagar_comisiones_vendedor_por_caja(db_session):
    admin = crear_usuario_admin(db_session)
    vendedor = crear_vendedor(db_session)
    comisiones = _crear_comisiones_pendientes(
...
def test_pagar_comisiones_vendedor_por_cuenta_bancaria(db_session):
    admin = crear_usuario_admin(db_session)
    vendedor = crear_vendedor(db_session)
    _crear_comisiones_pendientes(db_session, vendedor, admin, [(Decimal("1.00"), Decimal("2.00"), Decimal("3"))])
...
def test_pagar_comisiones_vendedor_requiere_exactamente_un_origen(db_session):
    admin = crear_usuario_admin(db_session)
    vendedor = crear_vendedor(db_session)
    _crear_comisiones_pendientes(db_session, vendedor, admin, [(Decimal("1.00"), Decimal("2.00"), Decimal("1"))])
    caja = crear_caja(db_session)
...
def test_pagar_comisiones_vendedor_sin_comisiones_pendientes_falla(db_session):
    ...
    with pytest.raises(ValueError, match="No hay comisiones pendientes"):
        PagoComisionService.pagar_comisiones_vendedor(
            db_session,
            id_vendedor=vendedor.id_vendedor,
            metodo_pago="efectivo",
