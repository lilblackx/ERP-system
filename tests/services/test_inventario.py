...
def test_actualizar_producto_codigo_duplicado(db_session):
    admin = crear_usuario_admin(db_session)
    categoria = crear_categoria(db_session)
    ProductoService.crear(db_session, **_datos_producto(categoria.id_categoria, cod_producto="SKU-001", creado_por=admin.id_usuario))
    otro = ProductoService.crear(
        db_session,
        **_datos_producto(categoria.id_categoria, cod_producto="SKU-002", nombre_producto="Otro", creado_por=admin.id_usuario),
    )
...
def test_cambiar_estado_producto_desactiva(db_session):
    ...
    actualizado = ProductoService.cambiar_estado(db_session, producto.id_producto, "INACTIVO", id_usuario=admin.id_usuario)
...
def test_establecer_precio_crea(db_session):
    ...
    precio = PrecioService.establecer_precio(db_session, producto.id_producto, "DETAL", "15.00", id_usuario=admin.id_usuario)
...
def test_establecer_precio_actualiza_existente(db_session):
    ...
    actualizado = PrecioService.establecer_precio(db_session, producto.id_producto, "DETAL", "20.00", id_usuario=admin.id_usuario)

    assert actualizado.precio_venta == Decimal("20.00")
    precios = PrecioService.listar_precios(db_session, producto.id_producto, id_usuario=admin.id_usuario)
...
def test_establecer_precio_tipo_invalido(db_session):
    ...
    with pytest.raises(ValueError, match="tipo_precio invalido"):
        PrecioService.establecer_precio(db_session, producto.id_producto, "OTRO", "10.00", id_usuario=admin.id_usuario)
...
def test_establecer_precio_costo_cero_margen_cero(db_session):
    ...
    precio = PrecioService.establecer_precio(db_session, producto.id_producto, "DETAL", "10.00", id_usuario=admin.id_usuario)
...
def test_listar_precios_multiples_tipos(db_session):
    ...
    producto = crear_producto(db_session, costo_producto=Decimal("10.00"))
    PrecioService.establecer_precio(db_session, producto.id_producto, "DETAL", "15.00", id_usuario=admin.id_usuario)
    PrecioService.establecer_precio(db_session, producto.id_producto, "MAYOR", "12.00", id_usuario=admin.id_usuario)
...
def test_listar_precios_sin_usuario_autorizado_falla(db_session):
    admin = crear_usuario_admin(db_session)
    producto = crear_producto(db_session, costo_producto=Decimal("10.00"))
    PrecioService.establecer_precio(db_session, producto.id_producto, "DETAL", "15.00", id_usuario=admin.id_usuario)

    with pytest.raises(PermisoDenegadoError):
        PrecioService.listar_precios(db_session, producto.id_producto)
...
def test_eliminar_precio(db_session):
    ...
    assert PrecioService.listar_precios(db_session, producto.id_producto, id_usuario=admin.id_usuario) == []
