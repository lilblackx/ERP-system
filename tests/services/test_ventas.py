...
from app.db.models import ComisionFactura, CuentaPorCobrar, FacturaDetalle, FacturaVenta
...
from tests.factories import crear_caja, crear_cliente, crear_producto
...
def test_anular_factura_con_pago_aplicado_bloqueada(db_session):
    producto = crear_producto(db_session, cantidad_unidad=50)
    cliente = crear_cliente(db_session, limite_credito=1000)
    ...
    PagoService.registrar_pago_cobro(
        db_session, id_cuenta_por_cobrar=cxc.id_cuenta_por_cobrar, monto=Decimal("10.00"), metodo_pago="efectivo", id_caja=caja.id_caja
    )
...
def test_anular_factura_sin_motivo(db_session):
    ...
    with pytest.raises(ValueError, match="motivo"):
        VentaService.anular_factura(db_session, factura.id_factura, id_usuario=None, motivo="")
...
def test_anular_factura_ya_anulada(db_session):
    ...
    VentaService.anular_factura(db_session, factura.id_factura, id_usuario=None, motivo="Motivo 1")

    with pytest.raises(ValueError, match="ya esta anulada"):
        VentaService.anular_factura(db_session, factura.id_factura, id_usuario=None, motivo="Motivo 2")
...
def test_listar_facturas_filtra_por_cliente(db_session):
    ...
    resultado = VentaService.listar_facturas(db_session, id_cliente=cliente_a.id_cliente)
