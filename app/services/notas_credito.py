...
class NotaCreditoService:
    ...
    def crear_nota_credito_cliente(
        ...
        # El nucleo _crear_nota_credito_cliente() sigue sin su propio require_permiso --
        # es un efecto secundario interno de una accion ya autorizada, y
        # VentaService.anular_factura() lo llama directo (no pasa por aca) bajo su propio
        # require_permiso("ventas", "eliminar"). Este metodo PUBLICO si lo exige (migracion
        # 0048): es el punto de entrada que quedaria expuesto sin permiso si algun dia se
        # agrega un callsite de UI que lo llame directo, en vez de via anulacion.
        require_permiso(session, id_usuario, "notas_credito", "eliminar")
    ...
    def crear_nota_credito_proveedor(
        ...
        # Mismo criterio que crear_nota_credito_cliente() (ver su comentario): el nucleo
        # privado no exige permiso (CompraService.anular_compra() ya lo hizo con
        # "compras"/"eliminar"), pero este metodo publico si (migracion 0048).
        require_permiso(session, id_usuario, "notas_credito", "eliminar")
